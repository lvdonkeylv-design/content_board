...
## 更新摘要
**变更内容**   
- 新增依赖管理基础设施章节，详细介绍 requirements.txt 标准化版本管理
- 更新依赖关系分析部分，包含精确的版本要求和间接依赖说明
- 增强环境部署指南，提供标准化的安装和配置流程
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖管理与版本控制](#依赖管理与版本控制)
7. [依赖关系分析](#依赖关系分析)
8. [性能与稳定性考量](#性能与稳定性考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录：技术选型说明](#附录技术选型说明)
...
- **标准化依赖管理**通过 requirements.txt 确保开发环境一致性
...
- **requirements.txt 统一管理所有第三方依赖及其精确版本**
...
L --> G
M["requirements.txt<br/>依赖版本管理"] --> K
```
...
- **依赖管理器**：通过 requirements.txt 标准化所有第三方包版本，确保跨环境一致性。
...
- [step2_1_table_to_html.py:39-68](file://step2_1_table_to_html.py#L39-68)
...
### 标准化依赖管理
项目现已引入完整的依赖管理机制，通过 `requirements.txt` 文件统一管理所有第三方包的精确版本，确保开发、测试和生产环境的一致性。

#### 直接依赖
项目定义了 5 个核心直接依赖，每个都指定了精确版本号：

- **selenium==4.46.0**：用于无头浏览器截图，支持最新的 ChromeDriver 集成
- **python-docx==1.2.0**：Word 文档解析库，提供稳定的 .docx 文件处理能力
- **opencv-python==5.0.0.93**：高性能图像处理库，用于封面裁剪和压缩
- **numpy==2.5.1**：数值计算基础库，为 OpenCV 提供底层支持
- **requests==2.34.2**：HTTP 客户端库，用于 Azure OpenAI 和微信公众号 API 调用

#### 间接依赖管理
requirements.txt 还记录了所有间接依赖的精确版本，包括：
- **网络相关**：urllib3==2.7.0、certifi==2026.6.17、charset-normalizer==3.4.9
- **异步框架**：trio==0.33.0、trio-websocket==0.12.2、sniffio==1.3.3
- **XML 处理**：lxml==6.1.1（python-docx 依赖）
- **类型注解**：typing_extensions==4.16.0
- **WebSocket 通信**：websocket-client==1.9.0、wsproto==1.7.1
...
- 核心第三方库
  - python-docx==1.2.0：解析 .docx 文档结构与样式
  - requests==2.34.2：HTTP 客户端（Azure OpenAI 代理、微信公众号 API）
  - selenium==4.46.0 + ChromeDriver：无头浏览器截图
  - opencv-python==5.0.0.93 + numpy==2.5.1：图像处理（裁剪、压缩、缩放）
...
```mermaid
graph LR
Py["Python 3.x"] --> Docx["python-docx==1.2.0"]
Py --> Req["requests==2.34.2"]
Py --> Sel["selenium==4.46.0 + ChromeDriver"]
Py --> CV["opencv-python==5.0.0.93 + numpy==2.5.1"]
Py --> CT["ctypes"]
...
- **依赖版本稳定性**
  - 通过 requirements.txt 锁定所有依赖版本，避免版本冲突导致的运行时错误。
  - 定期更新依赖版本以获取安全补丁和新功能。
...
- **依赖安装问题**
  - 使用 `pip install -r requirements.txt` 而非单独安装各个包。
  - 如遇编译错误，检查 Python 版本是否与依赖包兼容。
  - 网络问题导致下载失败时，可使用国内镜像源：`pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple`
...
## 结论
content_board 通过清晰的步骤化设计与稳健的错误处理，实现了从 Word 到微信公众号草稿箱的一键式内容生产流水线。其技术选型兼顾易用性与扩展性：python-docx 负责结构化解析，requests 对接外部服务，Selenium 解决复杂渲染截图，OpenCV 保障图片合规，ctypes 打通 Windows 剪贴板，HTML/CSS 模板与 JSON 数据格式贯穿全链路。**新增的标准化依赖管理机制确保了开发环境的一致性和可重现性**。建议在后续迭代中持续优化提示词工程、截图稳定性与网络重试策略，以提升整体鲁棒性与吞吐能力。
...
- requests==2.34.2
  ...
  - 版本锁定：避免网络协议差异导致的兼容性问题。
- Selenium==4.46.0 + Chrome
...
- **requirements.txt 依赖管理**
  - 选择理由：标准化版本管理，确保跨环境一致性，简化部署流程。
  - 管理范围：包含直接依赖和间接依赖的完整版本锁定。